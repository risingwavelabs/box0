...
// Agent-level headers (X-Agent-Token) for send/receive/ack — set after registration
      ...
      inputSchema: {
        ...
        properties: {
          ...
          task: {
            type: "string",
            description:
              "Description of what you need the agent to do",
          },
          context: {
            type: "string",
            description:
          ...
          timeout: {
            type: "number",
            description:
              "Max seconds to wait for a response (default: 120, max: 300)",
          },
        },
        required: ["to", "task"],
      },
...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (name === "discover") {
    ...
    const agents = (result?.agents || [])
      .filter((a: any) => a.id !== AGENT_ID) // exclude self
      .map((a: any) => ({
        id: a.id,
  ...
  // --- delegate (agent-auth for send/receive/ack) ---
  if (name === "delegate") {
    const { to, task, context, timeout: userTimeout } = args as {
      to: string;
      task: string;
      context?: string;
      timeout?: number;
    ...
    // Send the request (agent-auth, no "from" — server derives it from token)
    const content: any = { task };
    ...
    while (Date.now() < deadline) {
      ...
      for (const msg of messages) {
        ...
        if (msg.type === "question") {
          // Return the question to Claude Code so it can answer
          const questionText =
            typeof msg.content === "string"
              ? msg.content
              : JSON.stringify(msg.content, null, 2);
    ...
    return {
      content: [
        {
          type: "text",
          text: `Timed out waiting for **${to}** to respond after ${timeoutSec}s (thread: ${threadId}). The agent may still be working — you can check later.`,
        },
      ],
    };
  }
...
// Poll inbox and push events to Claude Code (agent-auth)

...
async fn test_agent_crud() {
    ...
    // Create
    let a = client
        .register_agent("admin", "reviewer", "Code reviewer", "Review code.", "local", "auto", false)
        .await
        .unwrap();
...
async fn test_agent_workspace_isolation() {
    ...
    alice_client
        .register_agent("alice", "alice-agent", "", "Do stuff.", "local", "auto", false)
        .await
        .unwrap();
    ...
    bob_client
        .register_agent("bob", "bob-agent", "", "Do stuff.", "local", "auto", false)
        .await
        .unwrap();
...
async fn test_agent_ownership_permission() {
    ...
    alice_client
        .register_agent("team", "alice-agent", "", "Do stuff.", "local", "auto", false)
        .await
        .unwrap();
...
async fn test_inbox_roundtrip() {
    ...
    // Register agents
    client.register_agent("admin", "sender", "", "Send stuff.", "local", "auto", false).await.unwrap();
    client.register_agent("admin", "receiver", "", "Receive stuff.", "local", "auto", false).await.unwrap();
...
async fn test_started_message_flow() {
    ...
    client.register_agent("admin", "lead", "", "Lead agent.", "local", "auto", false).await.unwrap();
    client.register_agent("admin", "worker-1", "", "Worker agent.", "local", "auto", false).await.unwrap();
...
async fn test_cron_crud() {
    ...
    client
        .register_agent("admin", "seo-agent", "SEO checker", "Check SEO.", "local", "auto", false)
        .await
        .unwrap();
...
async fn test_cron_invalid_schedule() {
    ...
    client
        .register_agent("admin", "agent", "", "Do stuff.", "local", "auto", false)
        .await
        .unwrap();
...
#[tokio::test]
async fn test_task_status_on_done() {
    let (url, key, _tmp) = start_test_server().await;
    let client = admin_client(&url, &key);
